...
myApp.controller("sessionCtrl", ["$scope","session","users","$location",function ($scope, session, users, $location) {
        ...
        $scope.login_as = function (user_id) {
            ...
            var user = filter(function (p1__7302$HASH$) {
                    return $EQ$$STAR$(user_id, p1__7302$HASH$['id']);
                }, users.users)[0];
...
myApp.controller("profileCtrl", ["$scope","session","users",function ($scope, session, users) {
        ...
        $scope.save_profile = function () {
            ...
            return (function () {
                var coll7303 = users.users;
                for (var k7304 in coll7303) {
                    var user = coll7303[k7304];
...
myApp.controller("facultyCtrl", ["$scope","session","courses",function ($scope, session, courses) {
        ...
        $scope.edit_course_box = function (id) {
            ...
            $scope.buffer = select_keys(filter(function (p1__7305$HASH$) {
                        return $EQ$$STAR$(id, p1__7305$HASH$['id']);
...
myApp.controller("studentCtrl", ["$scope","session","courses",function ($scope, session, courses) {
        ...
        $scope.register_course = function (course_id) {
            return (function () {
                var coll7306 = courses.courses;
                for (var k7307 in coll7306) {
                    var course = coll7306[k7307];
        ...
        $scope.unregister_course = function (course_id) {
            return (function () {
                var coll7308 = courses.courses;
                for (var k7309 in coll7308) {
                    var course = coll7308[k7309];
                    if ($EQ$$STAR$(course_id, course['id'])) {
                        
                        (function () {
                            var coll7310 = course['registered'];
                            for (var n in coll7310) {
                                var student = coll7310[n];
...
myApp.controller("accountantCtrl", ["$scope","session","courses","users",function ($scope, session, courses, users) {
        ...
        $scope.registrations = (function () {
            var ret7312 = [];
            var coll7315 = courses.courses;
            for (var k7316 in coll7315) {
                var course = coll7315[k7316];
                var coll7313 = course.registered;
                for (var k7314 in coll7313) {
                    var student = coll7313[k7314];
                    ret7312.push(merge({'course_id' : course.id,'course_title' : course.title,'course_desc' : course.desc,'paid' : student.paid}, select_keys(filter(function (p1__7311$HASH$) {
                                        return (student['id'] == p1__7311$HASH$['id']);
                                    }, users.users)[0], ['id','name','username','dob','email'])));
                };
            };
            return ret7312;
         })();
        $scope.confirm_payment = function (student_id, course_id) {
            (function () {
                var coll7319 = courses.courses;
                for (var k7320 in coll7319) {
                    var course = coll7319[k7320];
                    if ($EQ$$STAR$(course_id, course['id'])) {
                        
                        var coll7317 = course.registered;
                        for (var k7318 in coll7317) {
                            var student = coll7317[k7318];
            ...
            return (function () {
                var coll7321 = $scope.registrations;
                for (var k7322 in coll7321) {
                    var registration = coll7321[k7322];
                    if ($EQ$$STAR$(student_id, registration['id'])) {
                        
                        if ($EQ$$STAR$(course_id, registration['course_id'])) {
                            
                            assoc$EXCL$(registration, 'paid', true);
                        };
                    };
                };
            })();
        };
        $scope.unconfirm_payment = function (student_id, course_id) {
            (function () {
                var coll7325 = courses.courses;
                for (var k7326 in coll7325) {
                    var course = coll7325[k7326];
                    if ($EQ$$STAR$(course_id, course['id'])) {
                        
                        var coll7323 = course.registered;
                        for (var k7324 in coll7323) {
                            var student = coll7323[k7324];
                            if ($EQ$$STAR$(student_id, student['id'])) {
            ...
            return (function () {
                var coll7327 = $scope.registrations;
                for (var k7328 in coll7327) {
                    var registration = coll7327[k7328];
                    if ($EQ$$STAR$(student_id, registration['id'])) {
                        
                        if ($EQ$$STAR$(course_id, registration['course_id'])) {
                            
                            assoc$EXCL$(registration, 'paid', false);
                        };
                    };
                };
            })();
...
myApp.service("courses", function () {
        ...
        this.add_courses = function () {
            ...
            return (function () {
                var coll7329 = courses;
                for (var k7330 in coll7329) {
                    var course = coll7329[k7330];
        ...
        this.update_course = function (new_course) {
            return (function () {
                var coll7331 = that.courses;
                for (var k7332 in coll7331) {
                    var course = coll7331[k7332];
        ...
        this.remove_course = function (id) {
            return (function () {
                var coll7333 = that.courses;
                for (var n in coll7333) {
                    var course = coll7333[n];
...
myApp.filter("count_paid_students", function () {
        return function (coll) {
            return count(filter(function (p1__7334$HASH$) {
                        return (true === p1__7334$HASH$['paid']);
                    }, coll));
        };
    });
var registered$QUEST$ = function (course, user_id) {
    ...
    return (1 === count(filter(function (p1__7335$HASH$) {
                return $EQ$$STAR$(user_id, p1__7335$HASH$['id']);
            }, course['registered'])));
};
var paid$QUEST$ = function (course, user_id) {
    /* Helper function.
         Checks if a course is paid by current user. */
    return (1 === count(filter(function (p1__7336$HASH$) {
                return ($EQ$$STAR$(user_id, p1__7336$HASH$['id']) && (true === p1__7336$HASH$['paid']));
            }, course['registered'])));
};
...
myApp.filter("total_paid", function () {
        return function (courses, user_id) {
            return count(filter(function (p1__7337$HASH$) {
                        return paid$QUEST$(p1__7337$HASH$, user_id);
                    }, courses));
        };
    });
myApp.filter("total_registered", function () {
        return function (courses, user_id) {
            return count(filter(function (p1__7338$HASH$) {
                        return registered$QUEST$(p1__7338$HASH$, user_id);
                    }, courses));
        };
    });
var payment_percent = function (courses, user_id) {
    /* Helper function */
    return ((100 * (count(filter(function (p1__7339$HASH$) {
                return paid$QUEST$(p1__7339$HASH$, user_id);
            }, courses)) / count(filter(function (p1__7340$HASH$) {
                return registered$QUEST$(p1__7340$HASH$, user_id);
            }, courses)))) || 0);
};
